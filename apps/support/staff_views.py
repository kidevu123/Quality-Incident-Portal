...
class TicketWorkspaceView(LoginRequiredMixin, StaffUserMixin, DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.POST.get("action") == "zoho_replacement":
            from apps.zoho_integration.client import ZohoAPIError
            from apps.zoho_integration.services import (
                _format_zoho_error,
            ...
            if claim:
                try:
                    ...
                    messages.error(request, f"Zoho push failed: {_format_zoho_error(exc)}")
                except Exception as exc:  # noqa: BLE001
                    import logging
                    logging.getLogger(__name__).exception("Unexpected Zoho push failure")
                    messages.error(request, f"Zoho push failed: {exc}. See sync logs.")
            return redirect(reverse("ticket_workspace", kwargs={"public_id": self.object.public_id}))
